...
import { Package, Sparkles, UploadCloud } from 'lucide-react'
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="crextio-dark-card p-5 flex items-center justify-between">
          <div className="space-y-0.5">
            <span className="text-xs text-white/80 block font-medium">{t.products.actions}</span>
            <div className="text-sm font-bold text-white">{isZh ? '智能导入与批量管理' : 'Import & Bulk Manage'}</div>
          </div>
          <Link
            href="/dashboard/products/import"
            className="px-3.5 py-1.5 rounded-full bg-[#edbc40] hover:bg-[#B8F23A] text-[#111827] text-xs font-bold flex items-center gap-1.5 transition-all shadow-sm cursor-pointer"
          >
            <UploadCloud size={13} />
